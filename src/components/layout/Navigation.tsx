...
export default function Navigation() {
  ...
  useEffect(() => {
    const sections = [
      "home", "about", "skills", "projects", "sqlite-portable", "experience", "certificates", "contact"
    ];
    const handleScroll = () => {
  ...
  return (
    <nav className="fixed top-0 w-full bg-black/50 backdrop-blur-xl z-50 border-b border-white/5 transition-all duration-300">
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden px-6 py-4 bg-black/95 backdrop-blur-xl border-t border-white/5 flex flex-col gap-4 text-sm">
          {["Home", "About", "Skills", "Projects", "Experience", "Certificates", "Contact"].map((item) => (
            <a
              key={item}
              href={`#${item.toLowerCase()}`}
